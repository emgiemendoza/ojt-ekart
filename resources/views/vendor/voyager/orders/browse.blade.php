...
@section('page_header')
    <div class="container-fluid">
        ...
        @endcan --}}
        <a href="{{ route('orders.export') }}" class="btn btn-success btn-add-new">
            <i class="voyager-file-text"></i> <span>Export</span>
        </a>
                                    ...
                                    <tr class="{{ $data->error ? 'order-error' : '' }} {{ $data->shipped ? 'order-shipped' : '' }}">
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @if($row->type == 'image')
                                                    <img src="{{ url('storage') . '/' . $data->{$row->field} }}" style="width:100px">
                                                ...
                                                @elseif($row->type == 'multiple_images')
                                                    ...
                                                    @if($images)
                                                        ...
                                                        @foreach($images as $image)
                                                            <img src="{{ url('storage') . '/' . $image }}" style="width:50px">
                                                            {{-- <img src="@if( !filter_var($image, FILTER_VALIDATE_URL)){{ Voyager::image( $image ) }}@else{{ $image }}@endif" style="width:50px"> --}}
                                                        @endforeach
                                                    @endif
                                                @else
                                                    @include('voyager::multilingual.input-hidden-bread-browse')
                                                    @if ($row->display_name == 'Total' || $row->display_name == 'POS Total' || $row->display_name == 'Subtotal' || $row->display_name == 'Tax' || $row->display_name == 'Discount')
                                        ...
                                        <td class="no-sort no-click" id="bread-actions">
                                            <a href="{{ route('orders.exportSlip', $data->id) }}" title="eKart Slip" class="btn btn-sm btn-success">
                                                <i class="voyager-file-text"></i> <span class="hidden-xs hidden-sm">eKart Slip</span>
                                            </a>
                                            @can('delete', $data)
                                                @if($data->shipped == 0)
